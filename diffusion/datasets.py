...
class COCO2017DetectionWithCaptions(CocoDetection):
    """
    Extended COCO2017Detection that includes captions alongside labels.
    Returns a dict with 'image', 'labels', and 'captions'.
    """
    def __init__(self, root, annFile, captionFile, transform=None, target_transform=None):
        super().__init__(root, annFile, transform, target_transform)

        # Build class_to_idx
        cat_ids = sorted(self.coco.getCatIds())
        cats = self.coco.loadCats(cat_ids)
        self.class_to_idx = {cat['name']: i for i, cat in enumerate(cats)}
        ...
        for img_id in self.ids:
            ...
            labels = [self.class_to_idx[self.coco.cats[ann["category_id"]]["name"]] for ann in annots]
            file_to_class[base_name] = sorted(list(set(labels)))
    ...
    def __getitem__(self, index):
        ...
        label_list = sorted(set(self.class_to_idx[self.coco.cats[ann["category_id"]]["name"]] for ann in ann_list))
        captions = self.id_to_captions.get(img_id, ["No caption available"])
        return {'image': img, 'labels': label_list, 'captions': captions}
...
_json_path = osp.join(osp.dirname(__file__), 'imagenet_class_index.json')
...
def get_target_dataset(name: str, train=False, transform=None, target_transform=None):
    ...
    if name == "cifar10":
    ...
    elif name == "imagenet100": # ADD THIS BLOCK
        assert not train # Assuming this is for validation/testing
        # Use standard ImageFolder, it will infer classes from subdirs
        dataset = datasets.ImageFolder(root=osp.join(DATASET_ROOT, 'imagenet-100/val'),
                                    transform=transform,
                                    target_transform=target_transform)
        dataset.classes = get_classnames('imagenet100')
        dataset.file_to_class = None
    elif name == 'imagenet100_randomseed0':
        assert not train # Assuming this is for validation/testing
        # Use standard ImageFolder, it will infer classes from subdirs
        dataset = datasets.ImageFolder(root=osp.join(DATASET_ROOT, 'imagenet-100_randomseed0/val'),
                                    transform=transform,
                                    target_transform=target_transform)
        dataset.classes = get_classnames('imagenet100_randomseed0')
        dataset.file_to_class = None
    elif name == 'objectnet':
        ...
        dataset = MNIST(root=DATASET_ROOT, train=train, transform=transform, target_transform=target_transform,
                        download=True)
    ### add the coco 2017 dataset


    elif name == "coco2017":
        if train:
            ann_file = osp.join(DATASET_ROOT, "coco/annotations/instances_train2017.json")
            caption_file = osp.join(DATASET_ROOT, "coco/annotations/captions_train2017.json")
            img_dir = osp.join(DATASET_ROOT, "coco/images/train2017")
        else:
            ann_file = osp.join(DATASET_ROOT, "coco/annotations/instances_val2017.json")
            caption_file = osp.join(DATASET_ROOT, "coco/annotations/captions_val2017.json")
            img_dir = osp.join(DATASET_ROOT, "coco/images/val2017")

        dataset = COCO2017DetectionWithCaptions(
            root=img_dir,
            annFile=ann_file,
